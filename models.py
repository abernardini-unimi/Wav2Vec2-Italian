...
class Wav2Vec2ForSpeechClassification(Wav2Vec2PreTrainedModel):
    ...
    def merged_strategy(self, hidden_states, mode="mean"):
        if mode == "mean":
            ...
            raise ValueError("Pooling mode must be 'mean', 'sum', or 'max'.")
        return outputs
    ...
    def forward(
        ...
        if labels is not None:
            # (Il calcolo della loss nel tuo codice originale è corretto)
        ...
        if not return_dict:
            output = (logits,) + (outputs[2:] if isinstance(outputs, tuple) else ())
